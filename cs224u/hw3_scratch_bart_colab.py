...
class BARTRecogsModel(pl.LightningModule):
    def __init__(
        self,
        src_vocab_path,
        tgt_vocab_path,
        ...
        self.encdec = BartForConditionalGeneration(self.config)
        ...
        self.encdec.model.encoder.embed_tokens = nn.Embedding(
            self.enc_tok.vocab_size,
            self.config.d_model,
            padding_idx=self.enc_tok.pad_token_id,
            device=self.device,
        )
        self.encdec.model.decoder.embed_tokens = nn.Embedding(
        ...
        self.encdec.lm_head.weight = self.encdec.model.decoder.embed_tokens.weight
        self.encdec.final_logits_bias = torch.nn.Parameter(torch.zeros((1, self.dec_tok.vocab_size)))
        self.encdec.config.vocab_size = self.dec_tok.vocab_size
...
if __name__ == "__main__":
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath='/content/drive/MyDrive/CS224U/checkpoints',
        filename='bart-{epoch:02d}-{val_accuracy:.4f}',
        monitor='val_accuracy',
        mode='max',
